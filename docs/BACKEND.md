...
| GET | /api/admin/contacts | admin | `admin/contacts/route.ts`【F:src/app/api/admin/contacts/route.ts†L1-L77】 | Booking | `assertAdmin` (401 se non admin) | Contatti deduplicati per email con paginazione (`page` ≥1, `pageSize` ≤200) e filtri normalizzati; una sola chiamata Supabase (`admin_contacts_search_with_total`) restituisce righe + `total_count`, con logging strutturato in preview/dev.【F:src/lib/admin/contacts-service.ts†L1-L124】 |
...
| `GET /api/admin/contacts` | `assertAdmin` | Query `page` ≥1, `pageSize` ≤200, `q`, `newsletter`/`privacy` (`yes`\|`no`\|`all`), `from`/`to` (YYYY-MM-DD) | `200` → `{ data, total, page, pageSize }` (camelCase) | `500` errori DB | Singola chiamata Supabase `admin_contacts_search_with_total` (ritorna righe + `total_count`), logging preview/dev | `src/app/api/admin/contacts/route.ts` |
...
- **Risposta**: `{ data: ContactDTO[], total: number, page: number, pageSize: number }` dove `ContactDTO` espone `name`, `email`, `phone`, `lastContactAt`, `createdAt`, `privacy`, `newsletter`, `bookingsCount`, `totalBookings` (camelCase). `lastContactAt`/`createdAt` derivano da `last_contact_at` e sono stringhe o `null`; `bookingsCount` = `totalBookings` = contatore numerico.【F:src/lib/admin/contacts-service.ts†L69-L109】
- **Mappatura snake_case → camelCase**: la stored procedure restituisce `last_contact_at`, `total_bookings`, `total_count`; il servizio converte date in stringa, mantiene booleani/null e popola `total` leggendo `total_count`.【F:src/lib/admin/contacts-service.ts†L69-L124】
- **Calcolo `total`**: riuso della colonna `total_count` presente in ogni riga della funzione `admin_contacts_search_with_total`; nessuna seconda query `count(*)`.【F:src/lib/admin/contacts-service.ts†L96-L124】
- **Logging preview/dev**: quando `NODE_ENV !== 'production'` logga payload strutturato (`requestId`, `stage`, `queryNormalized`, `sqlArgs`, `durations`, `fingerprint`) per aiutare QA Supabase.【F:src/lib/admin/contacts-service.ts†L104-L124】
...
- `public.admin_contacts_search_with_total(search text, newsletter text, privacy text, from date, to date, limit int, offset int)` — stored procedure Supabase che ritorna righe e colonna `total_count`; l'API la invoca una sola volta via `prisma.$queryRaw` (runtime `nodejs`, `dynamic = 'force-dynamic'`).【F:src/app/api/admin/contacts/route.ts†L1-L77】
