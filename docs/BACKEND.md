...
### Admin Contacts API
- **Rotta principale** — `GET /api/admin/contacts` gira in runtime Node.js, forza risposte dinamiche (`dynamic = 'force-dynamic'`) e disattiva la cache (`revalidate = 0`).【F:src/app/api/admin/contacts/route.ts†L8-L22】 La risposta resta `{ items, page, pageSize, total, totalPages }` con DTO deduplicato dalla vista `admin_contacts_view` e ordinamento deterministico su `last_contact_at`/`email`.【F:src/lib/admin/contacts-query.ts†L93-L138】【F:src/app/api/admin/contacts/route.ts†L44-L72】
- **Diagnostica** — `GET /api/admin/contacts/_debug` verifica che l’utente sia admin, controlla la vista `public.admin_contacts_view` e ne restituisce conteggio + prime tre righe quando presente. In caso di errore logga `errorId` per i runtime logs.【F:src/app/api/admin/contacts/_debug/route.ts†L1-L89】
- **Errori mappati** — se la vista manca (codice Postgres `42P01` o Prisma `P2010`), la rotta principale risponde `501` con payload `{ ok:false, code:'MISSING_VIEW', next:'/api/admin/contacts/_debug' }`. Altri errori vengono tracciati con `errorId` e restituiti come `500 UNEXPECTED`.【F:src/app/api/admin/contacts/route.ts†L24-L80】
