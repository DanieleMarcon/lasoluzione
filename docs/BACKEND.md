...
| GET | /api/admin/contacts | admin | `admin/contacts/route.ts`【F:src/app/api/admin/contacts/route.ts†L1-L63】 | Booking | `assertAdmin` (401 se non admin) | Contatti deduplicati per email con filtri `q`, `newsletter`, `privacy`, `from`, `to`; paginazione (`page` ≥1, `pageSize` ≤200) e SQL raw parametrica (`ORDER BY "last_contact_at" DESC LIMIT … OFFSET`). Risposta `{ data, page, pageSize, total, hasNextPage, hasPrevPage }`.【F:src/lib/admin/contacts-query.ts†L1-L176】 |
...
| `/admin/contacts` | `ContactsPageClient` client + ToastProvider.【F:src/app/admin/(protected)/contacts/page.tsx†L1-L9】【F:src/components/admin/contacts/ContactsPageClient.tsx†L1-L330】 | Fetch `/api/admin/contacts` con DTO `{ data, page, pageSize, total, hasNextPage, hasPrevPage }` + query `q`, `newsletter`, `privacy`, `from`, `to`. | Deduplica per email, badge consensi, gestione errori leggibile e navigazione paginata lato client. |
...
| `GET /api/admin/contacts` | `assertAdmin` | Query `page`, `pageSize`, `q`, `newsletter`, `privacy`, `from`, `to` | `200` → `{ data, page, pageSize, total, hasNextPage, hasPrevPage }` | `401` se non admin | Deduplica prenotazioni per email con CTE, `ORDER BY "last_contact_at" DESC LIMIT/OFFSET` parametrico | `src/app/api/admin/contacts/route.ts` |
| `GET /api/admin/contacts/export` | `assertAdmin` | Query `q`, `newsletter`, `privacy`, `from`, `to` | `200` → CSV (max 10k righe, tag `# truncated` se eccede) | `401` se non admin | Re-usa filtri e DTO `fetchContactsData`, esporta CSV sanificato | `src/app/api/admin/contacts/export/route.ts` |
