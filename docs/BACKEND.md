...
| GET | /api/admin/contacts | admin | `admin/contacts/route.ts`【F:src/app/api/admin/contacts/route.ts†L1-L44】 | Booking | `assertAdmin` (401 se non admin) | Contatti deduplicati per email con paginazione (`page` ≥1, `pageSize` ≤100), filtro `q` su nome/email/telefono e ordine `createdAt DESC`; builder `Prisma.sql` condiviso con export/stampa e `$queryRaw` con binding tipizzati (niente `queryRawUnsafe`).【F:src/lib/admin/contacts-query.ts†L5-L156】 |
