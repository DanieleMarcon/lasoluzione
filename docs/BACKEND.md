...
| GET | /api/admin/contacts | admin | `admin/contacts/route.ts`【F:src/app/api/admin/contacts/route.ts†L4-L198】 | Booking | `assertAdmin` (401 se non admin) | Contatti deduplicati per email+telefono con paginazione (`page` ≥1, `pageSize` ≤200), filtro `q`/`search` case-insensitive su nome/email/telefono, filtri booleani `privacy`/`newsletter`, range `from`/`to`; SQL via `$queryRaw` parametrici con `ORDER BY last_contact_at DESC LIMIT … OFFSET …`. Risposta `{ data, page, pageSize, total, hasNextPage, hasPrevPage }`. |
...
| `/admin/contacts` | `ContactsPageClient` client + ToastProvider.【F:src/app/admin/(protected)/contacts/page.tsx†L1-L9】【F:src/components/admin/contacts/ContactsPageClient.tsx†L1-L312】 | Fetch `/api/admin/contacts` con DTO `{ data, page, pageSize, total, hasNextPage, hasPrevPage }` + query `q`, `newsletter`, `privacy`, `from`, `to`; gestione errori leggibili dal payload `{ error, detail }`. | Deduplica per email/telefono, badge consensi e navigazione paginata lato client. |
