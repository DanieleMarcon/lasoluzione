...
- **Pipeline**: l'handler tenta `public.admin_contacts_search_with_total(search, newsletter, privacy, from, to, limit, offset)` per ottenere righe e `total_count` in un'unica chiamata; gli argomenti sono sempre castati lato SQL (`::text`/`::date`/`::int`) per evitare errori di firma (`42883`). Il risultato viene wrappato in una select che fa `left join public.admin_contacts_hidden h on lower(v.email)=h.email` e `where coalesce(h.hidden,false)=false`, aggiungendo `count(*) over()` per aggiornare `total_count` ai soli contatti visibili. In assenza della funzione "with_total" si ricade su `public.admin_contacts_search(...)` più subquery `count(*)` con gli stessi parametri e lo stesso filtro, senza mai passare `NULL` a `limit`/`offset`. Se il join fallisce con `42P01` (tabella inesistente) viene creato `public.admin_contacts_hidden` via `CREATE TABLE IF NOT EXISTS` e la query viene ritentata automaticamente.【F:src/lib/admin/contacts-service.ts†L100-L267】
...
- **Logging**: in preview/dev (`NODE_ENV !== 'production'`) log strutturati con `requestId`, `stage` (`parse`, `sql:*`, `map`, `done`), `queryNormalized`, `sqlArgs`, `durations` e `fingerprint` degli errori; in produzione i log sono soppressi.【F:src/app/api/admin/contacts/route.ts†L17-L151】【F:src/lib/admin/contacts-service.ts†L19-L319】
- **Mutazioni**:
  - `PATCH /api/admin/contacts/[email]` (`assertAdmin`) — body `{ name?, phone? }`, aggiorna i campi sulle prenotazioni `public."Booking"` con match case-insensitive su `email`; ritorna `{ ok: true }` e logga `action: 'edit'`.【F:src/app/api/admin/contacts/[email]/route.ts†L50-L82】
  - `DELETE /api/admin/contacts/[email]` (`assertAdmin`) — soft delete: `INSERT ... ON CONFLICT` su `public.admin_contacts_hidden` con `hidden=true`, `hidden_at=now()`, `hidden_by=<admin>`; ritorna `{ ok: true }`, log `action: 'hide'`.【F:src/app/api/admin/contacts/[email]/route.ts†L84-L116】
  - `POST /api/admin/contacts/[email]/restore` (`assertAdmin`) — ripristina il contatto impostando `hidden=false` nella stessa tabella; ritorna `{ ok: true }`, log `action: 'restore'`.【F:src/app/api/admin/contacts/[email]/restore/route.ts†L33-L67】
- **Soft delete**: la tabella `public.admin_contacts_hidden(email text primary key, hidden boolean not null default true, hidden_at timestamptz not null default now(), hidden_by text)` viene creata on-demand (`CREATE TABLE IF NOT EXISTS`) e riutilizzata sia per il filtro delle liste/esportazioni sia per hide/restore; gli `email` vengono salvati in minuscolo e il join usa `lower(v.email)=h.email`.【F:src/lib/admin/contacts-service.ts†L7-L211】【F:src/app/api/admin/contacts/[email]/route.ts†L108-L118】
- **Log mutazioni**: in preview/dev (`NODE_ENV !== 'production'`) ogni chiamata PATCH/DELETE/POST produce `console.log({ action, targetEmail, by, ok, error? })` per audit manuale; in produzione resta silenzioso.【F:src/app/api/admin/contacts/[email]/route.ts†L32-L47】【F:src/app/api/admin/contacts/[email]/restore/route.ts†L19-L32】
...
- `public.admin_contacts_search_with_total(search text, newsletter text, privacy text, from date, to date, limit int, offset int)` — funzione preferita: restituisce righe + `total_count`; se non disponibile, l'API ricade su `public.admin_contacts_search(...)` con subquery `count(*)` separata (stessi parametri, niente `NULL` su `limit`/`offset`). Entrambe vengono wrappate con il join su `admin_contacts_hidden` e conteggio filtrato.【F:src/lib/admin/contacts-service.ts†L251-L319】
