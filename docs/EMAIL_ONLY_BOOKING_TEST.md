...
- Mailbox: due email (cliente: "Conferma la tua prenotazione" con link `/api/bookings/confirm?token=...`; admin: "Prenotazione in attesa").
   - Fallback: recuperare `token` da `BookingVerification` e costruire manualmente l'URL se necessario.

### 3. Conferma via link
1. Aprire il link di conferma dalla mail (`/api/bookings/confirm?token=...`).
...
- Link `/api/bookings/confirm?token=...` conferma la prenotazione e invia email di conferma a cliente e admin.
