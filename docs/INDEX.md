Aggiornato al: 2025-02-15

## Mini-TOC
- [Indice documentazione attiva](#indice-documentazione-attiva)
- [Archivio storico](#archivio-storico)
- [Mappa consolidamento (vecchio → nuovo)](#mappa-consolidamento-vecchio--nuovo)
- [Provenienza & Storia](#provenienza--storia)
- [Admin Contacts API (Supabase mapping)](./BACKEND.md#admin-contacts-api)

# Indice documentazione attiva
| Titolo | Path | Ultima modifica |
| --- | --- | --- |
| Documentazione La Soluzione | README.md | 2025-02-15 |
| Panoramica progetto & setup rapido | PROJECT_OVERVIEW.md | 2025-02-15 |
| Index documentazione | INDEX.md | 2025-02-15 |
| Architettura & Stato Sistema | ARCHITECTURE.md | 2025-02-15 |
| Backend, API & Data Layer | BACKEND.md | 2025-02-15 |
| Frontend, Routing & UX | FRONTEND.md | 2025-02-15 |
| Pagamenti & Integrazione Revolut | PAYMENTS.md | 2025-02-15 |
| Deployment — Vercel | DEPLOYMENT.md | 2025-02-15 |
| Workflow & Environment Guide | WORKFLOW_AND_ENVIRONMENT_GUIDE.md | 2025-02-15 |
| Known Issues — La Soluzione | KNOWN_ISSUES.md | 2025-02-15 |
| Roadmap — La Soluzione | ROADMAP.md | 2025-02-15 |
| Testing & QA | TESTING.md | 2025-02-14 |
| Catalogo & Carrello — Stato e Schema | CART_NOTES.md | 2025-10-14 |
| Prenotazione Solo Email — Guida e QA | BOOKING_EMAIL_ONLY.md | 2025-10-14 |
| Audit Responsività Mobile — 2025-02-14 | RESPONSIVE_AUDIT.md | 2025-10-14 |
| Changelog | CHANGELOG.md | 2025-10-14 |
| DevOps, Ambiente & Sicurezza Operativa | DEVOPS.md | 2025-10-14 |

# Archivio storico
| Titolo | Path | Ultima modifica |
| --- | --- | --- |
| Architettura & Stato Sistema | ARCHITECTURE.md | 2025-10-14 17:40 |
| Backend, API & Data Layer | BACKEND.md | 2025-10-14 17:50 |
| Prenotazione Solo Email — Guida e QA | BOOKING_EMAIL_ONLY.md | 2025-10-14 17:33 |
| Catalogo & Carrello — Stato e Schema | CART_NOTES.md | 2025-10-14 17:34 |
| Changelog | CHANGELOG.md | 2025-10-14 17:42 |
| Known Issues & Fix Log | KNOWN_ISSUES.md | 2025-10-14 17:55 |
| DevOps, Ambiente & Sicurezza Operativa | DEVOPS.md | 2025-10-14 17:51 |
| Frontend, Routing & UX | FRONTEND.md | 2025-10-14 17:37 |
| (senza titolo) | INDEX.md | 2025-10-14 17:51 |
| Pagamenti & Integrazione Revolut | PAYMENTS.md | 2025-10-14 17:32 |
| Panoramica progetto & setup rapido | PROJECT_OVERVIEW.md | 2025-10-14 17:51 |
| Documentazione La Soluzione | README.md | 2025-10-14 17:48 |
| Audit Responsività Mobile — 2025-02-14 | RESPONSIVE_AUDIT.md | 2025-10-14 17:47 |
| Testing & QA | TESTING.md | 2025-10-14 17:43 |
| Area amministrazione | _archive/ADMIN.md | 2025-10-14 13:50 |
| API Catalogo / Carrello | _archive/API_CART.md | 2025-10-14 13:50 |
| (senza titolo) | _archive/AUDIT_BACKEND.md | 2025-10-14 13:50 |
| Audit Catalogo / Carrello – fase 1 | _archive/AUDIT_CART.md | 2025-10-14 13:50 |
| (senza titolo) | _archive/AUDIT_PROMPT2.md | 2025-10-14 13:50 |
| Autenticazione (NextAuth v5) | _archive/AUTH.md | 2025-10-14 13:50 |
| Build & deploy (Vercel) | _archive/BUILD_AND_DEPLOY.md | 2025-10-14 13:50 |
| Cart Schema Notes | _archive/CART_SCHEMA_NOTES.md | 2025-10-14 13:50 |
| Changelog | _archive/CHANGELOG_ROOT.md | 2025-10-14 13:50 |
| Checkout flow | _archive/CHECKOUT_FLOW.md | 2025-10-14 13:50 |
| Database & Prisma | _archive/DATABASE.md | 2025-10-14 13:50 |
| Guida sviluppo | _archive/DEV_GUIDE.md | 2025-10-14 13:50 |
| Email & notifiche | _archive/EMAIL.md | 2025-10-14 13:50 |
| Percorso di Test "Prenotazione Solo Email" | _archive/EMAIL_ONLY_BOOKING_TEST.md | 2025-10-14 13:50 |
| Ambiente & variabili | _archive/ENVIRONMENT.md | 2025-10-14 13:50 |
| MVP — Prenotazione via email (singolo evento) | _archive/EVENTI_EMAIL_ONLY_MVP.md | 2025-10-14 13:50 |
| API Eventi pubblica | _archive/EVENTS_PUBLIC_API.md | 2025-10-14 13:50 |
| Operations runbook | _archive/OPERATIONS.md | 2025-10-14 13:50 |
| Archivio documentazione | _archive/README.md | 2025-10-14 17:43 |
| La Soluzione | _archive/README_ROOT.md | 2025-10-14 17:48 |
| Report refresh documentazione | _archive/REPORT_DOCS_REFRESH.md | 2025-10-14 13:50 |
| Report Audit Documentazione — 2025-10-10 | _archive/REPORT_DOCS_REFRESH_ROOT.md | 2025-10-14 13:50 |
| Integrazione Revolut Checkout | _archive/REVOLUT.md | 2025-10-14 13:50 |
| Routing & endpoint | _archive/ROUTES.md | 2025-10-14 13:50 |
| Sicurezza | _archive/SECURITY.md | 2025-10-14 13:50 |
| Stato snapshot – ottobre 2025 | _archive/STATE-SNAPSHOT.md | 2025-10-14 13:50 |
| State Snapshot — 2025-10-10 | _archive/STATE-SNAPSHOT_ROOT.md | 2025-10-14 13:50 |
| Test plan | _archive/TEST_PLAN.md | 2025-10-14 13:50 |
| Troubleshooting | _archive/TROUBLESHOOTING.md | 2025-10-14 13:50 |
| Revolut Merchant Sandbox Audit | _archive/revolut-audit.md | 2025-10-14 13:50 |
| Archivio documentazione | _archive/README.md | 2025-10-14 |
| Area amministrazione | _archive/ADMIN.md | 2025-10-14 |
| API Catalogo / Carrello | _archive/API_CART.md | 2025-10-14 |
| Audit backend (pre-merge) | _archive/AUDIT_BACKEND.md | 2025-10-14 |
| Audit Catalogo / Carrello – fase 1 | _archive/AUDIT_CART.md | 2025-10-14 |
| Audit prompt 2 | _archive/AUDIT_PROMPT2.md | 2025-10-14 |
| Autenticazione (NextAuth v5) | _archive/AUTH.md | 2025-10-14 |
| Build & deploy (legacy) | _archive/BUILD_AND_DEPLOY.md | 2025-10-14 |
| Cart Schema Notes | _archive/CART_SCHEMA_NOTES.md | 2025-10-14 |
| Changelog storico | _archive/CHANGELOG_ROOT.md | 2025-10-14 |
| Checkout flow (legacy) | _archive/CHECKOUT_FLOW.md | 2025-10-14 |
| Database & Prisma (legacy) | _archive/DATABASE.md | 2025-10-14 |
| Guida sviluppo (legacy) | _archive/DEV_GUIDE.md | 2025-10-14 |
| Email & notifiche | _archive/EMAIL.md | 2025-10-14 |
| Prenotazione solo email — test | _archive/EMAIL_ONLY_BOOKING_TEST.md | 2025-10-14 |
| Ambiente & variabili | _archive/ENVIRONMENT.md | 2025-10-14 |
| MVP prenotazione email | _archive/EVENTI_EMAIL_ONLY_MVP.md | 2025-10-14 |
| API eventi pubblica | _archive/EVENTS_PUBLIC_API.md | 2025-10-14 |
| Operations runbook | _archive/OPERATIONS.md | 2025-10-14 |
| Report refresh doc | _archive/REPORT_DOCS_REFRESH.md | 2025-10-14 |
| Audit documentazione 2025-10-10 | _archive/REPORT_DOCS_REFRESH_ROOT.md | 2025-10-14 |
| Integrazione Revolut (legacy) | _archive/REVOLUT.md | 2025-10-14 |
| Routing & endpoint | _archive/ROUTES.md | 2025-10-14 |
| Sicurezza | _archive/SECURITY.md | 2025-10-14 |
| State snapshot – ottobre 2025 | _archive/STATE-SNAPSHOT.md | 2025-10-14 |
| State snapshot root | _archive/STATE-SNAPSHOT_ROOT.md | 2025-10-14 |
| Test plan (legacy) | _archive/TEST_PLAN.md | 2025-10-14 |
| Troubleshooting | _archive/TROUBLESHOOTING.md | 2025-10-14 |
| Revolut Merchant Sandbox Audit | _archive/revolut-audit.md | 2025-10-14 |

# Mappa consolidamento (vecchio → nuovo)
| Documento legacy | Documento attivo | Note |
| --- | --- | --- |
| `_archive/REVOLUT.md`, `_archive/revolut-audit.md` | `PAYMENTS.md` | Integrate sequenza, webhook, stati booking. |
| `_archive/DEV_GUIDE.md`, `_archive/CHECKOUT_FLOW.md`, `_archive/ROUTES.md`, `_archive/ADMIN.md` | `FRONTEND.md` | Nuove sezioni tabella prenotazioni, filtri, errori runtime. |
| `_archive/AUDIT_BACKEND.md`, `_archive/DATABASE.md`, `_archive/API_CART.md`, `_archive/AUTH.md`, `_archive/EMAIL.md`, `_archive/EVENTS_PUBLIC_API.md` | `BACKEND.md` | Tabelle API complete, schema Prisma inline, CORS, errori. |
| `_archive/BUILD_AND_DEPLOY.md` | `DEPLOYMENT.md` | Aggiornata con engine Node 20.x e checklist post-deploy. |
| `_archive/ENVIRONMENT.md`, `_archive/OPERATIONS.md` | `WORKFLOW_AND_ENVIRONMENT_GUIDE.md` | Procedure passo-passo, mermaid flow, checklist. |
| Nuovo consolidamento bug (audit 2025-10) | `KNOWN_ISSUES.md` | Priorità, log, riproduzione. |
| Nuova pianificazione 2025 | `ROADMAP.md` | Milestone con criteri di done e impatto deploy. |

## Provenienza & Storia
SORGENTE: `docs/INDEX.md` (2025-10-14) + aggiornamenti hardening 2025-02-15  
COMMIT: 9d9f5c3  
MOTIVO DELLO SPOSTAMENTO: aggiornare elenco file attivi, aggiungere sezione mapping richiesto dal consolidamento.  
DIFFERENZE CHIAVE: nuove voci attive (DEPLOYMENT, KNOWN_ISSUES, ROADMAP), sezione "Mappa consolidamento" esplicita.
