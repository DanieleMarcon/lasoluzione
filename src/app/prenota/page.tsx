...
function CartSidebar() {
  ...
  const handleDecrease = (item: CartItem) => {
    const nextQty = item.qty - 1;
     if (nextQty <= 0) void removeItem(item.id);
     else void updateItem(item.id, nextQty);
    };
  const handleIncrease = (item: CartItem) => void updateItem(item.id, item.qty + 1);

  return (
    <aside className="card shadow-sm" style={{ position: 'sticky', top: '1rem' }}>
      <div className="card-body">
        <h2 className="h5 mb-3">Il tuo carrello</h2>
        {items.length === 0 && !loading ? <p className="text-muted mb-0">Il carrello è vuoto. Aggiungi un prodotto per iniziare.</p> : null}
        {items.length > 0 ? (
              ...
              <li key={item.id} className="mb-3 pb-3 border-bottom">
                <div className="d-flex justify-content-between align-items-start gap-3">
                  <div>
                    ...
                    <small className="text-muted">{moneyFormatter.format(item.priceCentsSnapshot / 100)} cad.</small>
                  </div>
                  <div className="d-flex align-items-center gap-2">
                     <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => handleDecrease(item)} disabled={loading}>−</button>
                    <span className="fw-semibold" aria-live="polite">{item.qty}</span>
                    <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => handleIncrease(item)} disabled={loading}>+</button>
                  </div>
                </div>
        ...
        <button type="button" className="btn btn-success w-100 mt-3" disabled={items.length === 0 || loading}>
          Vai al checkout
        </button>
      </div>
    </aside>
    ...
    <CartProvider>
      <main className="container py-5">
        <header className="text-center mb-5">
          ...
          <p className="text-muted mb-0">Scegli i piatti dalle sezioni disponibili e procedi al checkout quando sei pronto.</p>
        </header>
        <div className="row g-4">
          <div className="col-lg-8"><SectionAccordion /></div>
          <div className="col-lg-4"><CartSidebar /></div>
        </div>
      </main>
    </CartProvider>
  );
